...
final currentUserRoleProvider = StreamProvider<RoleData?>((ref) {
  ...
  // Debug logging
  debugPrint('[RoleService] AUTH uid=${user?.uid} email=${user?.email}');

  // If no user is logged in, return null immediately
  if (user == null) {
    debugPrint('[RoleService] No user logged in');
  ...
  debugPrint('[RoleService] merchantId=$merchantId branchId=$branchId');

  // If merchant/branch IDs aren't set yet, return null
  if (merchantId == null || branchId == null) {
    debugPrint('[RoleService] Missing merchant or branch ID');
    return Stream.value(null);
  }

  final rolePath = 'merchants/$merchantId/branches/$branchId/roles/${user.uid}';
  debugPrint('[RoleService] Reading role from: $rolePath');
  ...
  return FirebaseFirestore.instance
      ...
      .snapshots()
      .map((doc) {
    debugPrint('[RoleService] Role doc exists: ${doc.exists}, data: ${doc.data()}');
    if (!doc.exists || doc.data() == null) {
      // No role document means no access
      debugPrint('[RoleService] No role document found for user');
      return null;
    }
    return RoleData.fromFirestore(doc.data()!);
  }).handleError((error) {
    // If there's a permission error, return null instead of crashing
    debugPrint('[RoleService] ❌ Error loading role: $error');
    return null;
  });
});
